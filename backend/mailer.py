...
def send_receipt_email(to_email: str, plan: str, receipt_id: str, when_iso: str, amount: str = "$0.00") -> bool:
    """Sends a MOCK payment receipt when a user switches plans. No real money
    ever moves - this is just a demo confirmation email, same best-effort
    pattern as send_verification_email (never blocks the plan switch).
    Returns True if actually sent over SMTP, False if it just logged to the
    console (SMTP not configured)."""
    body = (
        "Hi!\n\n"
        f"This confirms your plan change on What Can I Cook.\n\n"
        f"  Plan:        {plan.capitalize()}\n"
        f"  Receipt ID:  {receipt_id}\n"
        f"  Date:        {when_iso}\n"
        f"  Amount:      {amount} (demo only - no card was actually charged)\n\n"
        "This is a demo application. No payment provider is connected, so this "
        "email is a mock receipt for testing/demo purposes only.\n"
    )
    if not mail_configured():
        print(f"[mailer] SMTP not configured - mock receipt for {to_email}:\n{body}")
        return False

    msg = MIMEText(body)
    msg["Subject"] = f"Your receipt - What Can I Cook ({plan.capitalize()} plan)"
    msg["From"] = FROM_EMAIL
    msg["To"] = to_email
