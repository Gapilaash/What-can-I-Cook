...
PRO_PLAN_PRICE = "$7.99"  # display-only, demo checkout — keep in sync with public/index.html + app.js


def _create_and_send_receipt(user_id: int, email: str, plan: str) -> str:
    """Best-effort: a plan switch should never fail just because mail sending
    hiccuped. Persists the receipt row first (so "Download receipt" always
    works even if the email send fails), then emails it. Returns receipt_id."""
    receipt_id = f"DEMO-{secrets.token_hex(4).upper()}"
    when_iso = datetime.utcnow().isoformat(timespec="seconds") + "Z"
    amount = PRO_PLAN_PRICE if plan == "pro" else "$0.00"
    with get_conn() as conn:
        conn.execute(
            "INSERT INTO receipts (receipt_id, user_id, plan, amount, created_at) VALUES (?, ?, ?, ?, ?)",
            (receipt_id, user_id, plan, amount, when_iso),
        )
        conn.commit()
    try:
        mailer.send_receipt_email(email.strip().lower(), plan, receipt_id, when_iso, amount)
    except Exception as e:  # noqa: BLE001
        print(f"[mailer] failed to send receipt email to {email}: {e}")
    return receipt_id
...
def change_account_plan(req: PlanRequest, user=Depends(auth.get_current_user)):
    ...
    No card is charged and no payment provider is involved. On upgrade to
    Pro, we best-effort email a mock receipt (never blocks the switch) and
    persist it so it can be re-downloaded later as a PDF."""
    profile = auth.set_plan(user["id"], req.plan)
    profile["receipt_id"] = None
    if req.plan == "pro" and profile.get("email"):
        profile["receipt_id"] = _create_and_send_receipt(user["id"], profile["email"], "pro")
    return profile
...
def substitute(req: SubstituteRequest, response: Response, user=Depends(auth.get_current_user)):
    if not ai_helper.ai_available():
        raise HTTPException(503, "AI substitutions need a free GEMINI_API_KEY (see backend/.env.example).")
    usage = auth.check_and_use_ai_quota(user["id"])
    if usage["limit"] is not None:
        response.headers["X-AI-Usage"] = f'{usage["used"]}/{usage["limit"]}'
    try:
...
def freestyle_recipe(req: FreestyleRequest, response: Response, user=Depends(auth.get_current_user)):
    if not ai_helper.ai_available():
...
def how_to_cook_ai(req: HowToCookRequest, response: Response, user=Depends(auth.get_current_user)):
...
@app.get("/api/account/receipt/{receipt_id}/pdf")
def download_receipt_pdf(receipt_id: str, user=Depends(auth.get_current_user)):
    with get_conn() as conn:
        row = conn.execute(
    ...
    profile = auth.get_profile(user["id"])
    pdf_bytes = pdf_export.build_receipt_pdf(dict(row), profile)
    return _pdf_response(pdf_bytes, f"receipt-{receipt_id}")
