...
def _send_receipt_email(email: str, plan: str):
    """Best-effort: a plan switch should never fail just because mail sending hiccuped."""
    receipt_id = f"DEMO-{secrets.token_hex(4).upper()}"
    when_iso = datetime.utcnow().isoformat(timespec="seconds") + "Z"
    try:
        mailer.send_receipt_email(email.strip().lower(), plan, receipt_id, when_iso)
    except Exception as e:  # noqa: BLE001
...
def change_account_plan(req: PlanRequest, user=Depends(auth.get_current_user)):
    ...
    Pro, we best-effort email a mock receipt (never blocks the switch)."""
    profile = auth.set_plan(user["id"], req.plan)
    if req.plan == "pro" and profile.get("email"):
        _send_receipt_email(profile["email"], "pro")
    return profile
