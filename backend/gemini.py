...
FALLBACK_MODELS = [
    m.strip()
    for m in os.environ.get("GEMINI_FALLBACK_MODELS", "gemini-3.5-flash-lite,gemini-3.1-flash-lite").split(",")
    if m.strip()
]
    ...
    for model in models:
        for attempt in range(ATTEMPTS_PER_MODEL):
            ...
            if resp.status_code in (403, 404):
                # This specific model isn't available/permitted for this key
                # (e.g. an older model family restricted to keys that used it
                # before) -> don't give up entirely, just try the next model.
                break
            ...
            # 400 / 401 etc: retrying won't help (bad key, malformed request...)
