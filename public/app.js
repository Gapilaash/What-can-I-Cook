...
async function switchPlan(plan) {
  try {
    ...
    renderAccountBadge();
    renderProfileModal();
    // If a recipe is open right now, its Cook mode / Print lock state depends on
    // currentProfile.plan — re-render it immediately instead of waiting for a refresh.
    if (currentTab === "recipe" && currentRecipeModel) {
      showRecipePage(currentRecipeModel);
    }
...
const checkoutDownloadBtn = document.getElementById("checkoutDownloadBtn");
...
checkoutForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    checkoutSuccessMsg.textContent = updated.email
      ? `A receipt was emailed to ${updated.email}.`
      : "Your plan is now Pro.";
    if (updated.receipt_id) {
      checkoutDownloadBtn.href = `${API_BASE}/api/account/receipt/${encodeURIComponent(updated.receipt_id)}/pdf`;
      checkoutDownloadBtn.classList.remove("hidden");
    } else {
      checkoutDownloadBtn.classList.add("hidden");
    }
...
detectBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    // Always clear the photo after a detect attempt — success or failure — so the dropzone resets.
    resetPhotoPicker();
  }
});
...
function showRecipePage(m) {
  ...
  const isPro = currentProfile && currentProfile.plan === "pro";
  // Cook mode & Print are Pro-only. Free users see a locked button that opens the upgrade modal.
  const cookModeBtn = isPro
    ? `<button class="btn btn-primary" id="rpCookMode">Cook mode</button>`
    : `<button class="btn btn-primary" data-open-upgrade title="Upgrade to Pro to unlock Cook mode">🔒 Cook mode</button>`;
  const printBtn = isPro
    ? `<button class="btn btn-ghost" id="rpPrint">Print</button>`
    : `<button class="btn btn-ghost" data-open-upgrade title="Upgrade to Pro to unlock Print">🔒 Print</button>`;
...
function bindRecipePage(m) {
  ...
  const printBtnEl = document.getElementById("rpPrint");
  if (printBtnEl) printBtnEl.addEventListener("click", () => window.print());
  const cookModeBtnEl = document.getElementById("rpCookMode");
  if (cookModeBtnEl) cookModeBtnEl.addEventListener("click", () => openCookMode(m.name, m.steps));
